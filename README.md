...
**Nota de proceso (sesión 22-ago-2026, tarde):** Fede marcó "Estado del Modelo" como básica y fea a pesar de figurar como "completa" — señal correcta: el layout original (semáforo chico arriba a la izquierda, texto plano debajo, ~40% del canvas usado) no estaba a la altura de las otras 3 páginas ya pulidas. Se acordó con Fede el rediseño antes de tocar nada: semáforo a la izquierda, texto a la derecha, más una tarjeta KPI nueva con el recall destacado. Al ir a construir la tarjeta KPI como visual "Tarjeta" nativo (igual a las de Resumen Ejecutivo) se verificó el modelo de datos y no hay ninguna medida DAX de recall en `Dim_Ruta`/`Dim_Zona`/`Hechos_Paradas` — el 18% es un resultado de la evaluación del notebook, no calculable en vivo con los datos cargados en Power BI. Se le presentó la disyuntiva a Fede (cuadro de texto con el número fijo vs. cargar los datos de verdaderos/falsos positivos para una medida real vs. no agregar la tarjeta) y eligió el cuadro de texto estilo tarjeta — más rápido y honesto sobre qué es y qué no es. Se implementó: (1) semáforo agrandado de 40x220 a 90x340 con relleno rojo sólido real (no solo resaltado de texto) y texto blanco 24pt; (2) tarjeta KPI nueva (340x200) con "18%" en 40pt rojo bold, más etiqueta y aclaración — ajustadas de 10pt a 18pt/14pt en una segunda vuelta porque Fede las vio desproporcionadamente chicas contra el número grande; (3) los 3 párrafos movidos a una caja de 760x440 a la derecha, letra subida de 16 a 18pt; (4) título reposicionado a ancho completo (1160px), mismo patrón que las otras páginas. Dos problemas técnicos encontrados y resueltos en el camino: (a) al agrandar el textbox del semáforo apareció una forma roja duplicada detrás — un objeto viejo del mismo tamaño original que había quedado invisible bajo el nuevo — se identificó y se borró; (b) un intento de traer Power BI al frente con `open_application` abrió una instancia nueva en blanco en vez de la existente (violación del protocolo del proyecto de nunca abrir una segunda instancia) — se detectó de inmediato por el título "Sin título" y el diálogo de conexión a SQL Server que se disparó, se cerró sin guardar, y se continuó sobre la ventana original sin pérdida de trabajo.
